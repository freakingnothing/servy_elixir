defmodule Servy.HttpServer do
  ...
  def start(port) when is_integer(port) and port > 1023 do
    ...
    # `active: false` - receive data when we're ready by calling `:gen_tsp.recv/2`
  ...
  def accept_loop(listen_socket) do
    ...
    # Suspends (blocks) and waits for client connection. When connection
    # is accepted, `client_socket` is bound to a new client socket.
    {:ok, client_socket} = :gen_tcp.accept(listen_socket)

    IO.puts "Connection accepted!\n"

    # Receives the request and sens a response over the client socket.
  ...
  def write_response(response, client_socket) do
    ...
    # Close the client socket, ending the connection.
